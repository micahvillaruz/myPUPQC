...
Array.from(Invoices).forEach(function (raw) {
	...
	if (raw.img) {
		var avatar_ =
			`<img src='public/images/users/` + raw.img + `' alt='' class='avatar-xs rounded-circle me-2'>`
	} else {
...
isCount = new DOMParser().parseFromString(invoiceList.items.slice(-1)[0]._values.id, 'text/html')
