<section class="copyright-management">
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header d-flex align-items-center">
                <h5 class="card-title mb-0 flex-grow-1">Copyright Management</h5>
            </div>

                <ul class="nav nav-pills nav-primary nav-custom-warning arrow-navtabs nav-justified mb-3" role="tablist">
                    <li class="nav-item">
                        <a class="nav-link active" data-bs-toggle="tab" href="#c_research" role="tab">
                            Research
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" data-bs-toggle="tab" href="#c_capstone" role="tab">
                            Capstone
                        </a>
                    </li>
                </ul>

            <div class="card-body">
                <div class="tab-content">
                    <div class="tab-pane active" id="c_research" role="tabpanel">
                            <table id="research-copyright-management-datatable" class="table table-bordered dt-responsive nowrap table-striped align-middle text-center" style="width: 100%">
                                    <thead>
                                        <tr>
                                            <th class="t-align">Research Title</th>
                                            <th class="t-align">Copyright Document</th>
                                            <th class="t-align">Status</th>
                                            <th class="t-align">Actions</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                    </tbody>
                                </table>
                    </div>
                    <div class="tab-pane" id="c_capstone" role="tabpanel">
                            <table id="capstone-copyright-management-datatable" class="table table-bordered dt-responsive nowrap table-striped align-middle text-center" style="width: 100%">
                                    <thead>
                                        <tr>
                                            <th class="t-align">Research Title</th>
                                            <th class="t-align">Copyright Document</th>
                                            <th class="t-align">Status</th>
                                            <th class="t-align">Actions</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                    </tbody>
                            </table>
                    </div>
                </div>
            </div>

        </div>
    </div>
</section>