<div>
    <div class="row mb-2">
        <div class="col-lg-12 w-100 mx-auto">
        </div>
    </div>
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header d-flex align-items-center">
                    <h5 class="card-title mb-0 flex-grow-1">Student Organizations</h5>
                    <div>
                        <button class="btn btn-primary btn-lg float-start mb-1 fs-5" data-bs-toggle="modal" data-bs-target="#add-organization-modal" id="add-organization"><i class="bx bx-plus"></i> Add New Organization</button>
                    </div>
                </div>
                <div class="card-body">
                    <table id="organizations-datatable" class="table table-bordered table-responsive dt-responsive nowrap table-striped align-middle text-center" style="width: 100%">
                        <thead>
                            <tr>
                                <th>Organization #</th>
                                <th>Organization Name</th>
                                <th>Abbreviation</th>
                                <th>Organization Description</th>
                                <th>Status</th>
                                <th>Action</th>
                            </tr>
                        </thead>
                        <tbody>
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
        <!--end col-->
    </div>
</div>

<!-- Add Organization Modal -->

