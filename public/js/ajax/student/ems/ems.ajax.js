...
fetchApprovedEvents = () => {
	...
	if (dt.length) {
		dt.DataTable({
			...
			bDestroy: true,
			ajax: {
				url: apiURL + `ems/student/view/organization_reservation`,
				type: 'GET',
				headers: AJAX_HEADERS,
				dataSrc: (result) => {
					const data = result.data
					const filteredData = data.filter((item) => {
						return item.reserve_status == 'Approved'
					})
					return filteredData
				},
			},
		...
		dt.DataTable({
			...
			ajax: {
				...
				dataSrc: (result) => {
					const data = result.data
					const filteredData = data.filter((item) => {
						return item.reserve_status == 'Pending'
					})
					return filteredData
				},
			},
