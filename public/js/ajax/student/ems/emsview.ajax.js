...
viewReservationDetails = () => {
	...
	$.ajax({
		type: 'GET',
		cache: false,
		url: apiURL + `ems/student/view/reservation`,
		dataType: 'json',
		success: (result) => {
			console.log(result)
			...
			let organization_name = userData.organization_assigned_to_reservation.organization_name
...
cancelReservation = (reservation_id) => {
	...
	$('#cancel-reservation').on('click', function () {
		$.ajax({
			url: `${apiURL}ems/student/cancel/reservation/${reservation_id}`,
			type: 'PUT',
			ContentType: 'application/x-www-form-urlencoded',
            dataType: 'json',
			success: (result) => {
				...
				if (result) {
					Toast.fire({
						...
						window.location.replace(`${baseURL}/student/ems/org-analytics`)
					})
				}
			},
			error: function (xhr) {
