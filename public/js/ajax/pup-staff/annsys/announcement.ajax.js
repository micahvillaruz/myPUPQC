			...
			columns: [
				...
				// * Action
				{
					...
					render: (data) => {
                        ...
                        `
						const editButton = `
                        <button type="button" class="btn btn-warning btn-icon waves-effect waves-light" onclick="editAnnouncement('${data.announcement_id}')">
...
updateAnnouncement = () => {
	if ($('#NewAnnouncement')[0].checkValidity()) {
		...
		$.ajax({
			url: apiURL + 'annsys/pup_staff/edit_announcement/' + $('#announcement_id').val(),
			type: 'PUT',
			headers: AJAX_HEADERS,
			data: data,
			dataType: 'json',
...
editAnnouncement = (announcement_id) => {
	$.ajax({
		url: apiURL + 'annsys/pup_staff/get_announcement/' + announcement_id,
		type: 'GET',
		headers: AJAX_HEADERS,
		success: (result) => {
			if (result) {
