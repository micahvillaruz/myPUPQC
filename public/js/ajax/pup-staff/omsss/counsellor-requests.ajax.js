$(function () {
	loadGuidanceRequestTable()

	$('#updateGuidanceAppointmentStatusModal').on('submit', function (e) {
		e.preventDefault() // prevent page refresh

		// pass data to API for updating of student's info
		editGuidanceAppointmentStatus($('#edit_health_appointment_id').val())
	})
})
...
loadGuidanceRequestTable = () => {
	...
	if (dt.length) {
        ...
        <button type="button" class="btn btn-info btn-icon waves-effect waves-light" onclick="viewGuidanceDetails('${data.health_appointment_id}')" data-bs-toggle="modal" data-bs-target="#viewGuidanceModal"><i class="ri-eye-fill fs-5"></i></button>
				<button type="button" class="btn btn-warning btn-icon waves-effect waves-light" onclick = "editGuidanceAppointmentDetails('${data.health_appointment_id}')" data-bs-toggle="modal" data-bs-target="#updateGuidanceAppointmentStatusModal" ><i class="ri-edit-2-fill fs-5"></i></button>
				</div>`
					},
				},
			],
...
getAppointment = (health_appointment_id) => {
	...
	}).fail(() => console.error('There was an error in retrieving counseling request data'))
}

editGuidanceAppointmentDetails = (health_appointment_id) => getAppointment(health_appointment_id)
// Edit Student Details
editGuidanceAppointmentStatus = (health_appointment_id) => {
	if ($('#updateGuidanceAppointmentStatusForm')[0].checkValidity()) {
		// no validation error
		const form = new FormData($('#updateGuidanceAppointmentStatusForm')[0])
			...
			success: (result) => {
				...
				if (result) {
					Swal.fire({
						...
						$('#updateGuidanceAppointmentStatusModal').modal('hide')
